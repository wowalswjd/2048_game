...
var 데이터 = [];
var 점수표 = document.getElementById('score').textContent;
...
function 랜덤생성() {
    ...
    if (빈칸배열.length === 0) {
        ...
        데이터[랜덤칸[0]][랜덤칸[1]] = 2; // 랜덤으로 2가 빈칸에 생성됨
        그리기();
    }
}
...
초기화();
랜덤생성();
그리기(); //데이터를 받아서 화면에 표시해주는 함수

/* swipe, 마우스 드래그 (마우스 누르기 - 방향 정하기 - 마우스 떼기) */
var 드래그시작 = false; // 아무데서나 mousemove되는거 막기 위해서 flag변수
var 드래그중 = false;
var 시작좌표;
var 끝좌표;
window.addEventListener('mousedown', function(이벤트) {
    ...
    드래그시작 = true;
    시작좌표 = [이벤트.clientX, 이벤트.clientY]; // 시작좌표 변수에 저장
});
...
window.addEventListener('mouseup', function(이벤트) {
    ...
    드래그시작 = false;
    끝좌표 = [이벤트.clientX, 이벤트.clientY]; // 끝좌표 변수에 저장

    /* x차이, y차이를 이용하여 기울기 알아냄 - 위아래왼오 구분 가능 */
    if(드래그중) {
        var 방향;
